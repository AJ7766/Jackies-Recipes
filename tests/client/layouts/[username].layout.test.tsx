...
jest.mock('@/app/_components/Profile', () => ({ profile }:{profile:ProfilePropsOrNull}) => <div>ProfilePage: {profile ? 'Profile Data' : 'No Profile'}</div>);
jest.mock('@/app/_components/Masonary', () => ({ profile }:{profile:ProfilePropsOrNull}) => <div>Masonary: {profile ? 'Masonary Data' : 'No Masonary'}</div>);
...
describe('[username] layout', () => {
    ...
    test('initial render with a non valid username', async ()=>{
        ...
        fetchMock.mockResponses(
            [JSON.stringify({ message: "Couldn't find user" }), { status: 400 }]
        );
        (useAuth as jest.Mock).mockReturnValue({
            user: null,
            isAuthenticated: false,
        ...
        await waitFor(async()=>{
            expect(screen.getByText('NavBar')).toBeInTheDocument();
            expect(screen.getByText('Child Component')).toBeInTheDocument();
            expect(screen.queryByText('ProfilePage: No Profile')).not.toBeInTheDocument();
            expect(screen.queryByText('Masonary: No Masonary')).not.toBeInTheDocument();
    ...
    test('initial render with a valid username', async ()=>{
        fetchMock.mockResponses(
            [JSON.stringify({ message: "Success fetching profile", profileData: {username: 'testacc'} }), { status: 200 }]
        );
        (useAuth as jest.Mock).mockReturnValue({
            user: null,
            isAuthenticated: false,
