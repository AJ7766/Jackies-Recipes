...
describe('RegisterForm component', () => {
  ...
  const fillForm = async (email?: string, username?: string, name?: string, password?: string, confirmPassword?: string) => {
        fireEvent.change(screen.getByPlaceholderText('Email') as HTMLInputElement, { target: { value: email } });
        fireEvent.change(screen.getByPlaceholderText('Username') as HTMLInputElement, { target: { value: username } });
        fireEvent.change(screen.getByPlaceholderText('Full Name') as HTMLInputElement, { target: { value: name } });
        fireEvent.change(screen.getByPlaceholderText('Password') as HTMLInputElement, { target: { value: password } });
        fireEvent.change(screen.getByPlaceholderText('Confirm Password') as HTMLInputElement, { target: { value: confirmPassword } });
  };
...
describe('Interaction Tests', () => {
  ...
  test('user can fill inputs in the form', async () => {
    ...
    await fillForm("email@example.com", "username", "full name", "password123", "password123");

    expect(screen.getByPlaceholderText('Email')).toHaveValue("email@example.com");
...
test('submit with invalid username', async () => {
    ...
    await fillForm("email@example.com", "username123", "full name", "password123", "password123");    
    const submitButton = await screen.findByRole('button', { name: /register/i });

    expect(screen.getByPlaceholderText('Email')).toHaveValue("email@example.com");
    expect(screen.getByPlaceholderText('Username')).toHaveValue("username123");
    expect(screen.getByPlaceholderText('Full Name')).toHaveValue("full name");
    expect(screen.getByPlaceholderText('Password')).toHaveValue("password123");
    expect(screen.getByPlaceholderText('Confirm Password')).toHaveValue("password123");
    ...
    await waitFor(() => {
        expect(screen.getByText('Username can only contain letters.')).toBeInTheDocument();
    });
});
...
test('submit with a username less than 4 letters', async () => {
    ...
    await fillForm("email@example.com", "asd", "full name", "password123", "password123");    
    const submitButton = await screen.findByRole('button', { name: /register/i });
    ...
    await waitFor(() => {
        expect(screen.getByText('Username must be atleast 4 letters.')).toBeInTheDocument();
    });
});
...
test('submit with a passwod less than 6 characters', async () => {
    ...
    await fillForm("email@example.com", "username", "full name", "12345", "12345");    
    const submitButton = await screen.findByRole('button', { name: /register/i });
    ...
    expect(screen.getByPlaceholderText('Confirm Password')).toHaveValue("12345");
    ...
    await waitFor(() => {
        expect(screen.getByText('Password must be atleast 6 characters.')).toBeInTheDocument();
    });
});
...
test('submit with a passwod not matching password', async () => {
    ...
    await fillForm("email@example.com", "username", "full name", "password123", "password321");    
    const submitButton = await screen.findByRole('button', { name: /register/i });
    ...
    expect(screen.getByPlaceholderText('Full Name')).toHaveValue("full name");
    expect(screen.getByPlaceholderText('Password')).toHaveValue("password123");
    expect(screen.getByPlaceholderText('Confirm Password')).toHaveValue("password321");
...
test('submit with valid credentials', async () => {
    fetchMock.mockResponseOnce(
        JSON.stringify({ message: 'Your account email@example.com has been successfully created!' }),
        { status: 201 }
    );
    await act(async () => {
        render(<RegisterForm />);
    ...
    await waitFor(() => {
        expect(screen.getByText('Your account email@example.com has been successfully created!')).toBeInTheDocument();
    });
});
