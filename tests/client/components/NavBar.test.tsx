...
const searchInput = async (input: string) => {
  await act(async()=>{
    fireEvent.change(screen.getByPlaceholderText('Search...') as HTMLInputElement, { target: { value: input } });
...
describe('NavBar component', () => {
  ...
  test('user has a profile picture', async () => {
    (useAuth as jest.Mock).mockReturnValue({
      user: {username: 'testacc', userContent: {profilePicture: '/path/test.jpg'}},
      isAuthenticated: true,
      initializing: false,
    });
    render(<NavBar />);
    ...
    await waitFor(() => {
      ...
      expect(profileImg).toHaveAttribute('src', '/_next/image?url=%2Fpath%2Ftest.jpg&w=96&q=75');
    });
  });

  test('input in search bar & finding a user', async ()=>{
    fetchMock.mockResponses(
      [JSON.stringify({ success: true, existingUsers: [{ username: 'testacc' }] }), { status: 200 }],
  );
    (useAuth as jest.Mock).mockReturnValue({
        user: null,
        isAuthenticated: false,
    ...
    await waitFor(async()=>{
      ...
      expect(screen.queryByTestId('searchedUsersContainer')).toBeInTheDocument();
  ...
  test('input in search bar & no user', async ()=>{
    jest.spyOn(console, 'error').mockImplementation(() => {});
    fetchMock.mockResponses(
      [JSON.stringify({ success: true, message:'No existing user with username non-testacc`' }), { status: 404 }],
  );
    (useAuth as jest.Mock).mockReturnValue({
        user: null,
        isAuthenticated: false,
    ...
    await waitFor(async()=>{
      expect(screen.getByPlaceholderText('Search...')).toHaveValue("non-testacc");
    })
    expect(console.error).toHaveBeenCalledTimes(1);
  });
  ...
  await waitFor(() => {
      ...
      expect(screen.queryByTestId('searchedUsersContainer')).not.toBeInTheDocument();
  ...
  test('searched user has a profile picture', async () => {
    ...
    fetchMock.mockResponses(
      [JSON.stringify({ success: true, existingUsers: [{ username: 'testacc', userContent: { profilePicture: '/_next/image?url=%2Fpath%2Fto%2Fpic.jpg&w=96&q=75' } }] }), { status: 200 }]
    );
    ...
    await waitFor(() => {
      ...
      expect(profileImg).toHaveAttribute('src', '/_next/image?url=%2F_next%2Fimage%3Furl%3D%252Fpath%252Fto%252Fpic.jpg%26w%3D96%26q%3D75&w=96&q=75');
    });
  });
});